# Sources

Use this file to record every source considered during the project.

## Referencing Format

Recommended format:

```text
Author surname, Initial. (Year). Title. Publisher/Journal/Website. URL or DOI. Accessed date.
```

## Core Source Categories

### Cryptocurrency Data

- Yahoo Finance. (2026). Bitcoin USD (BTC-USD) historical data. Yahoo Finance. https://finance.yahoo.com/quote/BTC-USD/history/ Accessed 17 June 2026.

### Source Evaluation Artifact

- Source evaluation workbook: `research/Source_Evaluation_Tianlin_He.xlsx`
- Quick-read summary: `research/source-evaluation-summary.md`

### Cryptocurrency Volatility Forecasting

- Katsiampa, P. (2017). Volatility estimation for Bitcoin: A comparison of GARCH models. Economics Letters, 158, 3-6. https://doi.org/10.1016/j.econlet.2017.06.023
- Catania, L., Grassi, S., & Ravazzolo, F. (2019). Forecasting cryptocurrencies under model and parameter instability. International Journal of Forecasting, 35(2), 485-501. https://doi.org/10.1016/j.ijforecast.2018.09.005
- Kim, J.-M., Jun, C., & Lee, J. (2021). Forecasting the volatility of the cryptocurrency market by GARCH and stochastic volatility. Mathematics, 9(14), 1614. https://doi.org/10.3390/math9141614
- Shen, Z., Wan, Q., & Leatham, D. J. (2021). Bitcoin return volatility forecasting: A comparative study between GARCH and RNN. Journal of Risk and Financial Management, 14(7), 337. https://doi.org/10.3390/jrfm14070337
- Zahid, M., Iqbal, F., & Koutmos, D. (2022). Forecasting Bitcoin volatility using hybrid GARCH models with machine learning. Risks, 10(12), 237. https://doi.org/10.3390/risks10120237
- Dudek, G., Fiszeder, P., Kobus, P., & Orzeszko, W. (2024). Forecasting cryptocurrencies volatility using statistical and machine learning methods: A comparative study. Applied Soft Computing, 151, 111132. https://doi.org/10.1016/j.asoc.2023.111132
- Huang, Z.-C., Sangiorgi, I., & Urquhart, A. (2024). Forecasting Bitcoin volatility using machine learning techniques. Journal of International Financial Markets, Institutions and Money, 97, 102064. https://doi.org/10.1016/j.intfin.2024.102064
- Brauneis, A., & Sahiner, M. (2026). Crypto volatility forecasting: Mounting a HAR, sentiment, and machine learning horserace. Asia-Pacific Financial Markets, 33, 379-411. https://doi.org/10.1007/s10690-024-09510-6

### GARCH / Statistical Models

- Bollerslev, T. (1986). Generalized autoregressive conditional heteroskedasticity. Journal of Econometrics, 31(3), 307-327. https://doi.org/10.1016/0304-4076(86)90063-1
- Hansen, P. R., & Lunde, A. (2005). A forecast comparison of volatility models: Does anything beat a GARCH(1,1)? Journal of Applied Econometrics, 20(7), 873-889. https://doi.org/10.1002/jae.800
- Patton, A. J. (2011). Volatility forecast comparison using imperfect volatility proxies. Journal of Econometrics, 160(1), 246-256. https://doi.org/10.1016/j.jeconom.2010.03.034

### Machine Learning and Interpretability

- Breiman, L. (2001). Random forests. Machine Learning, 45, 5-32. https://doi.org/10.1023/A:1010933404324
- Hochreiter, S., & Schmidhuber, J. (1997). Long short-term memory. Neural Computation, 9(8), 1735-1780. https://doi.org/10.1162/neco.1997.9.8.1735
- Lundberg, S. M., & Lee, S.-I. (2017). A unified approach to interpreting model predictions. Advances in Neural Information Processing Systems, 30. https://arxiv.org/abs/1705.07874
- Molnar, C. (2025). Interpretable machine learning: A guide for making black box models explainable (3rd ed.). https://christophm.github.io/interpretable-ml-book/
